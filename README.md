...
- 任意の店舗ロゴ・背景画像の差し替え（カスタム画像 / 休憩中スライドショー対応）
- 完全日本語UI、フルスクリーン対応
- **2 画面対応（v2.0.0〜）**: HDMI 拡張モニター接続時に「ホール側（お客向け表示）」と「PC 側（操作用）」を分離。起動時にモニター選択ダイアログが表示され、毎回手動で割り当て。HDMI 抜き差しに自動追従、タイマー進行は中断ゼロ。HDMI なし環境では v1.3.0 と完全同等の単画面モードで起動
- **マルチ表示モード（v2.7.0〜）**: 大型大会向けに、巨大スクリーンへ**最大 4 トーナメントの独立時計を 2×2 で同時表示**。手元 PC の操作盤とキーボードで区画ごとに操作でき、停電・強制終了からの復元（そこから再開 / 経過を反映）にも対応。登録トーナメントのデータは読み込むだけで上書きしない
