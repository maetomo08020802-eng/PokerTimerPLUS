...
## トーナメント開始ボイス（tournament-start-voice）

トーナメント開始時に再生する任意選択のボイス。設定 → 音タブ「トーナメント開始ボイス」で選択（`electron-store` のグローバルキー `startVoice` に永続化、`'off'` 既定）。

| ファイル | 用途 | 出典 | ライセンス | 表記 |
| --- | --- | --- | --- | --- |
| shuffle-up-and-deal-female-1.mp3 〜 -4.mp3 | 開始ボイス（女性 1〜4） | Yu Shimomachi（PLUS2）自作 | 自社所有（本アプリ専用） | 不要 |
| shuffle-up-and-deal-male-clear-1.mp3 〜 -4.mp3 | 開始ボイス（男性 1〜4） | Yu Shimomachi（PLUS2）自作 | 自社所有（本アプリ専用） | 不要 |

> 上記 8 ファイルは **Yu Shimomachi（PLUS2）が自作した音声**（2026-06-24 前原確認）。第三者ソースを含まないため帰属表記・ライセンス許諾は不要。本アプリへの同梱・配布に制約なし。
...
→ 本アプリの配布形態（Yu Shimomachi による無料配布、NSIS / DMG インストーラへの埋込）は完全に許諾範囲内。
