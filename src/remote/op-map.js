'use strict';
// remote-control Phase 1a — 操作名 → dispatchClockShortcut が受け付ける eventLike 写像。
//
// Phase 0 スパイク（spike-remote-control/op-map.js）を src へ正式昇格したもの。写像は Phase 0 と完全一致
// （テスト tests/remote-control.test.js が担保）。この写像1本を「真実源」とし server / phone で共有する。
//
// 対応元: src/renderer/renderer.js の dispatchClockShortcut の switch(event.code) と修飾キー分岐。
...
const OPS = Object.freeze({
  ...
  // 危険操作（Phase 1b = スマホ側にも確認ダイアログ＝前原決定(a)。1a では confirm なしで写像のみ）
  resetDialog:     { code: 'KeyR' }                            // 単独R = リセット確認ダイアログを開く
  // ※ トーナメント切替/削除/設定はキー割当が無い or S=設定ダイアログのため、
  //   将来フェーズで個別の op として設計する（Phase 1a スコープ外）。
});

// 破壊的操作。Phase 1b でスマホ UI 側に確認を挟む対象（前原決定 (a): PC限定にはしない）。
const DANGEROUS = Object.freeze(new Set(['resetDialog', 'reentryMinus', 'addOnMinus', 'specialMinus', 'entryCancel']));

// 操作名 → hall:forwarded-key と同型の payload（renderer.js の remote:op リスナーが受ける形）。
// 未知 op は null（server 側で 400 破棄＝ホワイトリスト思想）。
