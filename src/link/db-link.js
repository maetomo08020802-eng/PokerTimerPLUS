...
let _notify = null;         // K3: renderer への通知（main.js が全 window へ broadcast する callback）
let _healthOk = true;       // K3: 連携の健全性（false=切断中。変化時のみ notify）
...
/**
 * K3: 連携の健全性を更新する（変化時のみ renderer へ notify）。
 * down 判定 = network（fetch 失敗/timeout）/ auth（プレーン 404 = 誤キー・未設定）/ 429（brief ④ 逐語）。
 * 400・404 not_found・409 等の API エラーは「サーバー到達済み」= 接続自体は健全（down にしない）。
 */
function _trackHealth(res) {
  const down = !!(res && res.ok === false &&
    (res.code === 'network' || res.code === 'auth' || res.httpStatus === 429));
  const ok = !down;
  if (ok !== _healthOk) {
    _healthOk = ok;
    _log(ok ? 'dblink:health-ok' : 'dblink:health-down');
    if (_notify) {
      try { _notify({ type: 'health', down: !ok }); } catch (_) { /* ignore */ }
    }
  }
}
...
/**
 * K3: 復帰 probe / 起動・接続時の DB 状態取得（renderer 駆動・読取のみ=書込レートに無関係）。
 * 成功なら health も ok に戻る。renderer は返った clock を反映アダプタへ渡す（DB が正）。
 */
async function probe(pcTournamentId) {
  const dbId = _linkedDbId(pcTournamentId);
  if (!dbId || !_configured()) return { ok: false, code: 'not_linked', error: '連携対象の大会がありません' };
  const res = await getClock(dbId);
  if (!res.ok) return res;
  return { ok: true, clock: res.clock || null };
}

/**
 * K3: チェック OFF = 配信停止（仕様4）。POST /clock/stop（楽観ロックなし・停止意思優先）→ 対応表の行削除。
 * - `clock_not_found` は成功扱い（冪等・止めるものが無いだけ）。
 * - 通信失敗でも行は削除する（OFF の意思を優先）。その場合 warning で「配信が残っている可能性」を明示
 *   （復帰後にもう一度 ON→OFF で回収できる）。
 */
async function stopLink(p) {
  const tournamentId = typeof p.tournamentId === 'string' ? p.tournamentId : '';
  const dbId = _linkedDbId(tournamentId);
  const removeRow = () => setTournamentLink({ tournamentId, enabled: false });
  if (!tournamentId) return { ok: false, error: '対象のトーナメントがありません' };
  if (!dbId || !_configured()) {
    // 紐づけが無い / 未設定なら行削除だけ（通信しない）
    const saved = removeRow();
    return { ok: true, links: saved.links };
  }
  ...
  _log('dblink:stop-failed');
  return {
    ok: true,
    links: saved.links,
    warning: '連携先に接続できなかったため、配信が残っている可能性があります（復帰後にもう一度 ON→OFF で停止できます）'
  };
}
...
async function linkAndInit(p) {
  ...
  if (initRes.code === 'clock_running') {
    ...
    // K3: 取得した clock を返す = renderer が反映アダプタで PC を DB に従わせる（仕様2）。
