...
連携 ON の大会で、**進行中のタイマーが突然「何も押していない状態」に巻き戻ることがある**不具合を修正しました（実店舗で発生報告）。原因は、通信が一時的に不安定な間にサーバー側へ古い「未スタート」情報が残り、復帰時に PC がそれへ合わせてしまうことでした。修正後は、PC のタイマーが進行中の場合は古い「未スタート」情報に従わず、逆に PC の現在状態をサーバーへ送り直して整合させます。連携 OFF の大会（既定）には一切影響ありません。

### Fixed
- 外部連携（お店のアプリ）: 通信断からの復帰時・接続時・他端末との書込競合時に、サーバー側の古い「未スタート/終了」状態へ進行中のタイマーが追従してリセットされる問題を修正（PC 進行中はローカルが優先され、サーバー側を上書き整合）
- 調査用ログを追加（`dblink:stale-skip` ほか）。万一の再発時に原因を即特定できます

---

## v2.10.0 — 2026-07-18

「お店のアプリとの連携」の第 2 弾です。連携を ON にした大会に限り、時計の状態に加えて**画面の表示内容（イベント名・賞金・テロップ・背景テーマ・ロゴなど）**もお店のアプリへ自動送信され、お店の表示ページ（TV 表示）やお客様のスマホでも PC と同じ見た目の情報が表示できるようになります。**この機能も既定 OFF** で、連携を設定しない限り**これまでどおり完全にオフライン**で動作します。

### 🖼 表示内容・ロゴの自動配信（連携 ON の大会のみ）

- PC で**イベント名 / サブタイトル / ゲーム種 / 賞金区分 / 賞金プール（GTD 注記込み）/ 配当（PAYOUTS）/ 平均スタック / テロップ / 背景テーマ**を変更すると、数秒〜10 秒ほどで連携先の表示に反映されます（変更した時だけ送る省通信設計・毎秒は送りません）。
- **ロゴ画像**も送信されます。300KB を超える PNG / JPEG は**自動で縮小して送信**します（SVG ロゴは連携先へは送信できないため、その旨を設定画面に日本語で案内します。PC 画面上のロゴ表示はこれまでどおりです）。
- テロップの**文字色装飾は PC 画面ではそのまま**表示されます（連携先には色なしの本文だけが流れます）。
- 背景テーマのうち「エメラルド」「オブシディアン」「背景画像」は、連携先では標準の「ネイビー」として表示されます（PC 画面はこれまでどおりです）。

### 🔒 送信内容についての大切なお知らせ（v2.9.0 からの変更）

- 連携 ON の大会では、時計の状態とブラインド構成に加えて、**画面に表示している賞金プール額・配当額（＝金銭の表示情報）を送信するようになりました**（店内 TV やお客様のスマホに表示するためです）。
- **氏名などの個人情報・PC 内の他の大会データ・スライドショー / 背景画像は引き続き一切送信しません**。送信先は**あなたが設定画面で入力した連携先だけ**です。
- 連携 OFF・未設定の大会と 4 分割マルチ表示は、これまでどおり**完全オフライン**で動作します（通信は一切発生しません）。

---

## v2.9.0 — 2026-07-18

新機能 **「お店のアプリとの連携（外部データベース連携）」** を追加しました。連携を設定した大会だけ、時計の状態（レベル・残り時間・人数）がお店のアプリへ自動送信され、お客様のスマホやお店の表示ページでも残り時間を見られるようになります。**この機能は既定 OFF** で、設定しない限り**これまでどおり完全にオフライン**で動作します（従来の単画面・2 画面・マルチ表示はすべて完全に同じ動作です）。

### 🔗 お店のアプリとの連携 — 時計の状態を自動配信

- 設定画面の「**外部連携**」タブで「**連携先 URL**」と「**店舗キー**」の 2 つを入力するだけで使えます（アカウント登録・ログインは不要です）。
- 連携チェックを ON にすると**今日の大会一覧**が表示され、1 つ選んで紐づけると、そのトーナメントの**ブラインド構成が連携先に登録**され、以後は**開始・一時停止・レベル送り・±30秒・人数変更**などの操作が自動で送信されます（毎秒は送らない省通信設計）。
- 紐づけはこの PC 内の対応表に保存されるだけで、**お店のアプリ側の大会名・設定には何も書き込みません**。
- 連携チェックを **OFF にすると配信は即終了**し、お客様のスマホ等から残り時間が消えます（再度 ON にすれば再開できます）。

### 📶 通信が切れても時計は止まりません

- 連携中にネットが切れると PC 画面の右上に「**⚠ 連携切断中（復帰待ち）**」と表示されますが、**タイマー本体は PC 内でそのまま動き続けます**（HDMI でつないだ会場モニターも止まりません。切断表示は手元 PC のみに出ます）。
- ネットが復帰すると**自動的に再接続**し、連携先の最新状態に合わせて同期し直します。
- 店舗キーを間違えている場合などは「連携先 URL または店舗キーを確認してください」と日本語で案内します。

### 🔒 プライバシー・通信についての大切なお知らせ

- 送信されるのは、**連携を ON にした大会**の**時計の状態（レベル・残り時間・人数）とブラインド構成だけ**です。**金銭情報・個人情報・PC 内の他の大会データは一切送信しません**。
- 送信先は**あなたが設定画面で入力した連携先だけ**です。アプリに特定の送信先が組み込まれていることはありません。
- **連携を設定しない限り、これまでどおり完全にオフライン**で動作します（既定は OFF・未設定時は外部への接続を一切行いません）。
- 店舗キーはこの PC 内に保存されます。キーを差し替えると古いキーは使えなくなります（紛失時はお店の管理者がキーを再発行してください）。
- マルチ表示モード（4 分割）は連携の対象外です。

---

## v2.8.0 — 2026-07-08

新機能 **「スマホ遠隔操作（実験的機能）」** を追加しました。会場で PC を離れても、手元のスマホから操作できます。**この機能は既定 OFF** で、OFF のときは**これまでどおり何も変わりません**（従来の単画面・2 画面表示・マルチ表示はすべて完全に同じ動作です）。

### 📱 スマホ遠隔操作（実験的機能）— PC を離れてもスマホで操作

- 設定画面の「**スマホ操作**」タブで ON にすると、**同じ Wi-Fi につないだスマホのブラウザ**から、リエントリー・アドオン・エントリー追加・脱落・タイマー操作（開始/一時停止・±30秒）・テロップ表示などを操作できます。
- 使い方: 「スマホ操作」タブを ON → 画面に表示された **QR をスマホのカメラで読む**（または URL を手入力）→ スマホの画面に**表示された PIN を入力**して接続 → ボタンで操作。
- スマホ側には**現在の人数・リエントリー数・アドオン数・卓名がリアルタイムで表示**されます。リセットなどの重要な操作は、スマホ側でも確認画面が出ます。
- 対象は**単画面（通常）表示モードのみ**です（マルチ 4 分割表示モードは対象外）。
- スマホ操作は **1 タップでできる操作の補助用**です。「開始」など PC 画面に設定ウィンドウが出る操作は、PC 側での操作も必要です（リモコンだけでは完結しません）。

### 🔒 プライバシー・通信についての大切なお知らせ

- これまで本アプリは**完全にオフライン**で動作していました。今回の「スマホ遠隔操作」を **ON にしたときだけ**、**同じ Wi-Fi 内のスマホとの通信**を行います。
- **インターネットや外部のサーバーへの送信は、引き続き一切行いません。**通信は会場内の同じ Wi-Fi の中だけに閉じています。
- 機能を **OFF にすれば、これまでどおり完全にオフライン**で動作します（既定は OFF）。
- 接続には毎回変わる PIN が必要で、無操作が続くと自動的に切断されます（安全のため）。

### ⚠️ ご利用にあたっての注意（会場の環境について）

- **初回だけ** ON にした直後に「Windows の許可（ファイアウォール）」が出ることがあります。「**アクセスを許可する**」を選んでください（管理者パスワードを求められる場合があります）。
- **スマホと PC を同じ Wi-Fi** につないでください。別のネットワークやスマホの回線（4G/5G）では接続できません。
- 会場の Wi-Fi が「**端末どうしの通信を遮断する設定（AP アイソレーション。ゲスト用 Wi-Fi に多い）**」の場合は、アプリ側では解決できず接続できません。その場合はスマホのテザリングや別の Wi-Fi をお試しください。
- 実験的機能のため、うまく動かない場合は OFF にしてご利用ください（従来動作に戻ります）。

---

## v2.7.0 — 2026-07-08

大型大会向けの新モード **「マルチ表示モード（4分割）」** を追加しました。単画面・従来の 1 トーナメント表示は**これまでどおり**で、何も変わりません。

### 🖥️ マルチ表示モード（4分割）— 最大 4 つのトーナメント時計を同時表示

- 設定画面の「ハウス情報」タブから開始すると、巨大スクリーン（HDMI 拡張モニター）に**最大 4 つのトーナメント時計を 2×2 で同時表示**できます。
- 各区画は**完全に独立した時計**です。開始時刻・ブラインド構造・一時停止・レベル進行がそれぞれバラバラに動き、1 区画の操作が他の区画に影響することはありません。
- 各区画には登録済みトーナメントを割り当てます（開始前カウントダウンも区画ごとに設定可能）。
- このモードで無効になるのは「休憩中スライドショー」「通知音」「テロップ」の 3 つだけです。

### 🎛️ 手元 PC の操作盤

- 手元 PC には専用の操作盤が表示され、区画ごとに **スタート / 一時停止 / レベル送り・戻し / リセット / ±30秒 / エントリー・脱落・リエントリー・アドオン** を操作できます。
- モニター複製（ミラーリング）で使う場合のために、**キーボードだけでも操作**できます（1〜4 キーで区画を選択、H キーで操作一覧を表示）。
- 空いている区画には「無地 / 店舗ロゴ / 任意の画像 / 固定テキスト」を表示できます（フィラー＝埋め草表示）。

### 🔌 トラブルに強い設計

- **停電・アプリ強制終了からの復帰**: 不意にアプリが終了しても、次回マルチ表示を開始するときに直前の状態を復元できます。復元時に「**そこから再開**（時計は進めない）」か「**経過を反映して復元**（停電中の時間も進んだ扱いにする）」かを選べ、ダイアログには前回終了からの経過時間（例: 終了から約42分）も表示されます。
- **HDMI 抜き差しに自動対応**: マルチ表示中に会場スクリーンのケーブルが抜けても操作盤とタイマーは動き続け、挿し直すと 2×2 表示が自動で復帰します。
- **誤操作ガード**: 進行中の区画を別のトーナメントに差し替えようとすると確認が出ます（うっかり操作でタイマーや人数が消えない）。
- **終了時の確認**: 進行中の区画があるままマルチ表示を終了（× ボタンや終了ボタン）しようとすると確認が出ます。停電などの異常終了と違い、正常な終了は復元できないため、うっかり × で進行が消えるのを防ぎます。

### 🔒 大切なデータはそのまま

- マルチ表示モードは登録済みトーナメントを**読み込むだけ**です。このモードでの進行や操作（タイマー・エントリー数など）が、登録トーナメントのデータに保存・上書きされることはありません（開始時に確認画面でもご案内します）。
- 単画面（HDMI 非接続）でのご利用、従来の 2 画面表示は**これまでと完全に同じ**動作です。

---

## v2.6.6 — 2026-06-24

テロップ編集の小さな分かりやすさ改善です。

### 🔎 色プレビューが「見るだけの欄」と分かるように

- 前バージョン（v2.6.5）で追加した、テロップの色付き表示の枠を「編集できる」と勘違いして触ってしまう方がいたため、枠の上に **「プレビュー（表示見本・ここは編集できません）」** と明示しました。
- 文字の編集は上の入力欄で、色付きの見本は下のプレビューで、と役割がはっきりします（枠の挙動はこれまでと同じです）。

---

## v2.6.5 — 2026-06-24

テロップ（画面下を流れる告知）の**文字色の変え方をぐっと簡単**にしました。

### 🎨 テロップの色を「文字を選んで色ボタン」で変えられるように

- これまでは表示テキスト欄に `[red]文字[/red]` のような**記号を手打ち**する必要がありましたが、**色を変えたい文字を選んで、上の色ボタンを押すだけ**で色が付くようにしました（「イベント名の色」と同じ操作感）。
- **9 色のボタン**＋**任意の色（カラーピッカー）**＋**「色を消す」**を用意。**編集中のプレビュー**で流れる前の仕上がりを確認できます。
- 設定の「テロップ」タブでも、`Ctrl+T` の編集画面でも、**同じように**操作できます。
- これまで `[red]…[/red]` の記法で作ったテロップは**そのまま表示・編集**できます（記法の手打ちも引き続き可能）。

---

## v2.6.4 — 2026-06-24

トーナメント開始時に流せる「開始ボイス」を追加しました。

### 🎙️ トーナメント開始ボイスを選べるように

- 設定 →「音」タブに **「トーナメント開始ボイス」** を追加しました。**なし＋女性4種＋男性4種**から選べます。
- トーナメントが始まった瞬間（**「今すぐ開始」も、カウントダウン終了時も**）に、選んだボイスが1回鳴ります（従来のスタート音と置き換え。二重には鳴りません）。
- 「試聴」ボタンでその場で聞き比べできます。
- この設定は**アプリ全体で共通**（全トーナメントで同じ）で、選んだ内容は次回起動後も保持されます。**「なし」**を選べば従来どおりの動作です。

---

## v2.6.3 — 2026-06-24

2 画面（HDMI 拡張モニター）利用時の操作反映の速さを改善しました。単画面でお使いの場合は影響ありません。

### ⚡ 手元 PC の操作が会場モニターに即座に反映されるように

- 走行中に手元 PC で**一時停止/再開（Space）・レベル切替・「今すぐ開始」**などの操作をした際、会場モニター（ホール側）への反映が最大で約 0.5 秒遅れることがありました。これを**ほぼ即時（体感で 0.1 秒未満）**に短縮しました。
- タイマーの数字の進み方や賞金計算など、これまでの挙動は一切変わりません。改善されるのは「手元の操作が会場画面に映るまでの速さ」だけです。
- 開始前カウントダウン（プレスタート）まわりの挙動は従来どおりで、退行はありません。

---

## v2.6.2 — 2026-06-24

テロップ（画面下を流れる告知文字）まわりの操作性アップです。

### ✨ テロップの表示/非表示を「T」キーで切り替え

- 設定画面を開かなくても、**キーボードの `T` キー**でテロップの表示↔非表示をワンタッチで切り替えられるようになりました（手元 PC で押すと会場モニターにも反映されます）。
- 文字入力中や設定ダイアログを開いている間は誤動作しないようになっています。`Ctrl+T`（テロップ編集ダイアログ）は従来どおりです。

### ✨ テロップの一部だけ色を変えられるように

- テロップ本文の一部を `[red]ここだけ赤[/red]` のように囲むと、**その部分だけ色を変えて**流せるようになりました。
- 使える色: `red` / `gold` / `yellow` / `orange` / `green` / `cyan` / `blue` / `pink` / `white`、または `#RRGGBB` 形式の指定。
- 編集欄の下に記法のヒントを表示しています。対応外の色や記法はそのまま文字として表示されます（壊れません）。

---

## v2.6.1 — 2026-06-23

開始前カウントダウン（プレスタート）まわりの小さな表示修正です。挙動の中身（タイマー進行・賞金計算）は変わりません。

### 🐛 開始前カウントダウン中の「NEXT BREAK IN」が正しい時間になりました

- トーナメント開始前のカウントダウン中、右上の「NEXT BREAK IN（次の休憩まで）」が誤った時間を表示し、カウントダウンが 0 になって実際にスタートした瞬間に正しい値へ切り替わる（ジャンプして直る）現象を修正しました。
- カウントダウン中も、**実際にスタートした直後と同じ「次の休憩までの時間」**を表示するようになり、開始の前後で値が連続します。

### ✨ 参加人数の初期値を 3 人にしました

- スタート時に開く参加人数入力ダイアログの初期値を、毎回 **3 人**から始まるように変更しました（実際の開始は 3〜4 人が大半のため）。±ボタンや直接入力での調整、1〜500 人の範囲はこれまでどおりです。

---

## v2.6.0 — 2026-06-08

賞金まわりを **店内通貨（$）ベース**に作り替えた大きな改修です。日本の実運用に合わせ、わかりにくい「％」をアプリから完全になくし、**金額（店内通貨）でそのまま入力**できるようにしました。**既存トーナメントの賞金プール総額（数値）は変わりません**（表示の通貨記号のみ ¥→$ に読み替え。独自に設定した記号はそのまま保持されます）。

### ✨ 賞金プールを「1件あたりの拠出額（店内通貨）× 件数」に

- これまでフィーに対する「反映率（％）」で決めていた賞金プールを、**フィー種別ごとの「1件あたり拠出額（店内通貨 $）」× 件数**で積み上げる方式に変更しました。
  - 例：「1エントリーで $3000 をプールに拠出」→ 件数が増えるほどプールが $3000 ずつ積み上がります。
- **$（整数）× 件数**なので、％にあった**1円単位の端数が原理的に出ません**。
- 拠出額は **¥フィー（実際の買込）とは独立**。¥フィーは「買込（店売上）の記録」として残し、自由に入力できます（プール計算には影響しません）。
- ハウス情報の店舗デフォルトも「1件あたり拠出（$）」に変更しました（新規トーナメント作成時の初期値）。

### ✨ 配当は「金額（店内通貨）」で固定入力 — ％は廃止

- 順位ごとの配当を **金額（店内通貨 $）でそのまま入力**します。**入れた金額は固定**で、開き直しても参加人数が変わっても**ズレません**（以前の「％設定だと開き直すと端数がズレる」問題の根治）。
- 配当の「％ / 金額」切替や、フィーの「反映率％」など、**アプリから「％」入力が完全になくなりました**。

### ✨ 賞金は店内通貨で扱う前提に — 鍵・警告モーダルを廃止

- 賞金・プール・配当はすべて**店内通貨（$）**で表示・運用します（現金で賞金を出すモードはありません）。
- フィー欄の **🔒ロック（解除確認ダイアログ）を廃止**しました。賞金プールがフィーと独立になったため、ロックの必要がなくなったためです。フィー欄はそのまま自由に編集できます。

### 🔄 既存データの移行（自動・安全）

- 旧「反映率％」は起動時に自動で「1件あたり拠出（$）」へ変換されます（100%→フィー額、0%→0）。**プール総額の数値は変わりません**。
- 通貨記号は既定の「¥」を「$」へ読み替えます（独自に設定済みの記号はそのまま保持）。
- 既存の％配当トーナメントも、開くと綺麗な金額で表示されます。

### ⚡ 動作を軽くしました（パフォーマンス改善）

- **設定画面（「S」キー）を開くと激重になる問題を解消**しました。設定ダイアログの背景ぼかし効果が、後ろで動き続けるタイマー画面を毎フレーム再計算し続けて重くなっていたため、ぼかしを外しました（背景は従来どおり暗くなり、設定は読みやすいままです）。
- **アプリ起動中に他のアプリまで重くなる問題**にも対処しました。全画面背景の描画方式の見直し、テロップの常駐描画ヒントの削減、手元（操作）ウィンドウが最小化中の省電力化を行い、グラフィックが非力な PC での負荷を下げています（会場モニター側の表示なめらかさは従来どおり維持）。

### ✨ スタック入力の整理 ＋ プリセット説明の追加

- バイインの初期スタックの入力欄を**「チップ（スタック）」1つに統一**し、紛らわしかった独立の「スタートスタック」欄をなくしました。AVG STACK（平均スタック）の計算結果は従来どおりで、**既存トーナメントの数値は変わりません**。
- 配当の**「プリセット適用」ボタンに説明**を追加しました（選んだ順位数の定番配分で各順位の金額を自動入力する、という説明）。

### 🔄 既存データの移行（自動・安全）— スタック

- スタック入力欄の統一にあたり、内部で初期スタックの値を引き継ぐため、**既存トーナメントの AVG STACK は数値が一切変わりません**（実データで移行前後の一致を検証済み）。

### 🛡 安全性

- 既存テスト全 PASS を維持（合計 **1358 件**）。タイマー・2画面同期・音声・ブランディング・レイアウト等の中核機能には影響しません。賞金プール計算・配当・スタックの移行はいずれも実データで検証済みで、**既存のトーナメント・設定はそのまま**です（自動更新でも破壊的影響はありません）。

---

## v2.5.1 — 2026-06-07

ブラインド構造の編集まわりと設定ダイアログを「分かりやすく・安全に」する改善をまとめてお届けします。**保存データの形式・移行は一切変わりません**（既存のトーナメント・設定・画像はそのまま保持されます）。

### ✨ 改善：設定タブの適用範囲を見える化 + 編集中トーナメント名の常時表示

- **設定タブを「適用範囲」で2グループに視覚分割**：
  - 「このトーナメント専用（選ぶたびに内容が変わる）」… トーナメント / ブラインド構造 / 背景・時計フォント / テロップ
  - 「アプリ全体で共通（全トーナメントで同じ）」… ロゴ / 音 / ハウス情報
  - 各タブの設定がそのトーナメント専用なのか、アプリ全体共通なのかが、押す前に一目で分かるようになりました。
- **設定ダイアログ上部に「編集中：◯◯（トーナメント名）」を常時表示**。トーナメントを切り替えると即座に更新されます（無名時はフォールバック表示）。
- **ブラインド構造タブに軽い注記**を追加（「全トーナメントで共有のテンプレート」である旨）。

### ✨ 改善：ブラインド構造の編集を「いま選んでいるトーナメント」に固定 + 共有時の保存確認

- ブラインド構造タブの編集対象を、**いま選択中のトーナメントが使う構造**に固定しました。無関係な他トーナメントの構造をうっかり編集してしまう混乱をなくしています（新規作成時に構造のひな形を選ぶ導線は従来どおり）。
- 編集した構造が**他のトーナメントでも使われている場合は、保存時に3択の確認**を表示します：
  - 「すべてに反映して保存」… 共有している全トーナメントに反映（使い回しの一括編集）。
  - 「このトーナメントだけ変更」… コピーを作って選択中だけ差し替え（他は元のまま）。
  - 「やめる」… 保存を中止（編集内容はそのまま保持）。
- 共有していない場合は、従来どおり確認なしで保存します。

### ✨ 改善：ブラインドのレベル表を見やすく + トーナメント一覧の折りたたみ

- **ブラインドのレベル表を縦に拡張**し、通常表示で約10段見えるようにしました。表示が多い構造でも「保存」「適用」「行を追加」ボタンが常に押せます（ヘッダー行は固定）。
- **トーナメント一覧を折りたたみドロップダウン化**。普段は「選択中の1件＋状態」だけを表示し、トグル（▼）で全件を開閉できます。トーナメントが増えても一覧で画面が埋まらず、いまどれが選択中かが一目で分かります（状態バッジ・各行の操作ボタンは従来どおり）。

### 🐞 修正：大きい画面でブラインドのレベル表が見えなくなる問題

- 大型モニター/TV など画面が広い環境で、**ブラインドのレベル表が極端に潰れて段数が見えなくなる**ことがある問題を根治しました。どの画面解像度でも表が0段に潰れず、最低限の段数が必ず見えるようにしています。

### 🐞 修正：未保存の編集中にトーナメントを切り替えたときの保護

- ブラインド構造を**未保存のまま編集中に別のトーナメントへ切り替えると、編集内容が黙って失われたり表示と実体がずれたりする**ことがある問題を修正しました。切り替え時に「未保存の編集を破棄しますか？」の確認（破棄して切り替える／やめる）を表示します。

### 🛡 既存機構の完全保持

- **データ構造・保存ロジック・migration は一切変更なし**（既存トーナメント・設定・画像をそのまま保持、自動更新でも破壊的影響なし）。
- 致命バグ保護 5 件すべて影響なし。タイマー進行・2画面同期・賞金プール計算の挙動は不変。
- `<dialog>` への flex 追加なし（v2.0.0 不変条件維持）/ レイアウトシフト撲滅 5 原則維持 / 設定 UI はホール側非表示（単画面後方互換維持）。
- 既存テスト全 PASS 維持 + 本リリースの新規テスト追加（合計 **1261 件** 全 PASS）。
