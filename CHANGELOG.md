...
設定ダイアログの「どの設定がどこに効くのか」を分かりやすくする UI 改善です。**機能・保存内容・データ形式は一切変わりません**（見た目とラベルのみの変更）。
...
- データ構造・保存ロジック・各タブの中身・タブ切替ロジック（`data-tab` 識別子）はすべて無変更。タブの並び替え・グループ化は表示のみ。
- 致命バグ保護 5 件すべて影響なし（UI/CSS/ラベル/現在名表示のみの変更）。
- `<dialog>` への flex 追加なし（v2.0.0 不変条件維持）/ レイアウトシフト撲滅 5 原則維持 / 設定 UI はホール側非表示（単画面後方互換維持）。
- 既存テスト全 PASS 維持（1180 件）+ 本変更の新規テスト 13 件追加（合計 1193 件）。
