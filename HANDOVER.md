...
| **v2.0.x 配布** | ❌ 未配布（main マージ済、タグ未作成、`.exe` 未公開）|
| **GitHub repo** | <https://github.com/maetomo08020802-eng/PokerTimerPLUS> |
| **テスト数** | **229 件全 PASS**（v2.0.4 で増加見込み）|
| **現在ブランチ** | `feature/v2.0.4-coverage`（main は v2.0.3 完成）|
| **直近フェーズ** | v2.0.3 完成（PR #6 マージ済）→ v2.0.4 進行中（CC 並列 3 体で残検証 D/E/I/J/K）|
| **次のステップ** | CC v2.0.4 完了 → CC_REPORT 採点 → PR #7 マージ → 配布判断 |
| **致命バグ保護** | 5 件すべて維持（C.2.7-A / C.2.7-D / C.1-A2 系 / C.1.7 / C.1.8）|
| **累計致命級発見・修正** | **3 件**（hall 差分購読 / hall 逆書き込み / PC 間データ移行）|
| **CC モデル運用** | **Sonnet 4.6 デフォルト**（Opus 4.6 は AUP False Positive 多発、要 memory 参照）|

### 新セッション構築士の最初のアクション
1. memory 全部読む（特に `feedback_cc_pitfalls.md` の「追記 2026-05-01」）
2. `poker-clock/CC_REPORT.md` を Read（CC v2.0.4 作業完了報告があるか確認）
3. `poker-clock/NEXT_CC_PROMPT.md` を Read（v2.0.4 指示内容）
4. 完了報告あり → 採点 + PR マージ案内 + 配布判断
5. 完了報告なし → 前原さんに「CC 作業中ですか? 完了したら教えてください」
