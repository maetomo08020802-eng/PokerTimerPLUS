...
- **テスト**: 1353件全PASS（version 2.6.0、perf +13 / stack-unify +12）。**統合テストインストーラ生成済（fee-pot-yen + perf + stack-unify 全部焼込）**: `dist\pokertimerplus-setup-2.6.0.exe`（≈83MB、--publish never、HEAD `4535f15`）。asar 焼込確認（stackModel=6 / チップ（スタック）=2 / starting-stack残留=0 / preset-hint=1 / bg-scroll=1 / operator-throttle=1 / potAmounts=42）＋ packaged 起動スモーク OK（8秒生存・クラッシュなし）。**main 非merge / tag 無し / GitHub Release 非接触（最新 Release は v2.5.1 のまま）**。
