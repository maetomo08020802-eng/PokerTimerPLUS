...
| 案件 | 状態 | 成果物 / 引継ぎ |
|------|------|--------|
| 案件230 K4: PC表示データ・ロゴ送信 | 🟡 実機確認待ち(v2.10.0 配信済) | report=`.cc-reports/2026-07-18_案件230_K4_PC表示データ送信.md`。実配信の実機確認(6-B ①〜⑦)=前原PC を v2.10.0 へ更新(再起動)→連携ON大会で表示変更→/tv・顧客反映 |
...
| オープン作業 | 案件230 K4=v2.10.0 配信済・実配信の実機確認(6-B)のみ残 |
...
- **現在地(2026-07-18)**: 案件230 K4=PC 表示メタ+ロゴ送信を **v2.10.0 として配信完了**。⚠️発覚した根因=K4 は main には入っていたが v2.9.0 リリース(tag が K4 merge より前)に含まれず、実機は自動更新で v2.9.0 実行=送信コード不在で「本番 tournament_clock_display に1件も届かない」症状(時計同期は v2.9.0 機能なので届いていた)→ v2.10.0 リリースで解消。完了review✅・push前diff review✅・テスト1864件全PASS・INVARIANT例外②改定済(金銭表示情報OK・前原承認)。**次=前原PCを v2.10.0 へ自動更新(再起動)→ 連携ON大会で表示変更→ /tv・顧客反映を実機確認(6-B①〜⑦)**。残提案=runtime 送信の pcId ガード・切断中ロゴ再送(report §8)。
