...
### 🔖 引き継ぎサマリ（2026-06-08 セッションクリア時点・最終）

**現在地: v2.6.0 配信済み・公開中（Latest）。legacy-id-rename（内部整理）main 取込済。オープン作業 0 件・安定運用フェーズ。**

- **ブランチ/HEAD**: `main`（origin 同期済）、**HEAD `fbf7203`**（直前: PROGRESS クローズ docs → その前が legacy-id-rename merge `95c7621`）。**作業ツリー clean**。version **2.6.0**。**テスト 1358 件全 PASS**、致命バグ保護 5 件全維持。
- **公開中の最新 Release = v2.6.0**（GitHub Release Latest・自動更新有効、merge `a1bce57`/tag `v2.6.0`、アセット latest.yml+setup.exe+.blockmap）。URL: https://github.com/maetomo08020802-eng/PokerTimerPLUS/releases/tag/v2.6.0
- **v2.6.0 の内容（4 ピース・全配信済）**: ① fee-pot-yen（賞金=店内通貨$・1件あたり拠出×件数／%全廃／配当は金額固定）② perf-heaviness（背景 fixed→scroll・marquee will-change 撤去・operator のみ backgroundThrottling、hall/operator-solo 据置）③ stack-unify+preset-hint（初期スタックを buyIn.chips に統一・スタートスタック欄廃止／プリセット説明）④ perf-dialog-backdrop（設定ダイアログ ::backdrop の blur 撤去）。**データ移行安全（実 store 13 件検証 mismatch=0／TOTAL POOL 数値不変）**。前原実機 6-B 全 OK + GO 後に配信。
- **legacy-id-rename（v2.6.0 後の内部整理、配信なし）**: UI 内部名 `*-pool-rate`→`*-pot` の純粋リネーム（DOM id/el-map/変数）。orphan0・null 参照0・1358 件全 PASS。前原 GO で main merge `95c7621`（`--no-ff`・**tag/Release なし・version 据置**）→ `chore/legacy-id-rename` 削除済。データキー `poolRates`/`poolRatesDefault`・CSS class `__pool-rate-*` は dormant/対象外で温存。
- **アーカイブ**: v2.6.0 関連 7 案件（payout-amount-default/fee-pot-yen/perf-heaviness/stack-unify-preset-hint/perf-dialog-backdrop/v260-release/legacy-id-rename）の md 46 件を `.cc-archive/v2.6.0-payout-model/{briefs,plans,reports}/` に統合退避。アクティブ `.cc-briefs|plans|reports` 直下は README/.gitkeep/_template のみ（クリーン）。merge 済 feature ブランチ（payout-amount-default / settings-scope-clarity / legacy-id-rename）削除済。
- **次にやること**: 特になし（安定運用）。次案件の brief 受領待ち。温存候補: v2.3.0（PRE_START 永続化、温存ブランチ `feature/v2.3.0-prestart-persistence`）/ CSS class `pool-rate-*`→`pot-*` の統一（別 brief 可）/ データキー poolRates の dormant 除去（全ユーザー POT 化確認後）/ 旧 v2.0.x・v2.1.x の merge 済ローカルブランチ多数の一括整理。
- **運用メモ**: マウント git 表示は不安定（git が authoritative）。配信（main merge/tag/push/Release）は前原 GO 必須。report 書出直後に `cc-kouchikushi2` 能動呼出で完了 review。`docs/RELEASE_GUIDE.md` 準拠だが実運用は CC 自走 CLI（build `--publish never` → `gh release create --latest`）。
