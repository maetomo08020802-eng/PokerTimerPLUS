...
| v2.5.1 | settings-scope-clarity 再テストビルド（バグ①修正反映版 .exe、STEP1〜4+修正 合算、--publish never・main非merge） | 🟡 実機確認待ち（前原 6-B 合算やり直し） | `.cc-briefs/2026-06-07_settings-scope-clarity_testbuild2_brief.md` | （ビルドのみ・plan省略） | `.cc-reports/2026-06-07_settings-scope-clarity_testbuild2.md` |
| v2.5.1 | settings-scope-clarity 配信前修正（ブラインド未保存編集中のトーナメント切替ガード） | 🟡 再テストビルド待ち（修正完了） | `.cc-briefs/2026-06-07_settings-scope-clarity_dirty-switch-guard_brief.md` | `.cc-plans/2026-06-07_settings-scope-clarity_dirty-switch-guard_plan.md` | `.cc-reports/2026-06-07_settings-scope-clarity_dirty-switch-guard.md` |
| v2.5.1 | settings-scope-clarity 再テストビルド3（dirty-switch-guard 反映版 .exe、配信前最終、--publish never・main非merge） | 🟡 実機確認待ち（前原 配信前最終 6-B） | `.cc-briefs/2026-06-07_settings-scope-clarity_testbuild3_brief.md` | （ビルドのみ・plan省略） | `.cc-reports/2026-06-07_settings-scope-clarity_testbuild3.md` |
...
| オープン作業 | 1 件（settings-scope-clarity STEP1〜4 + 実機バグ修正2件、v2.5.1、再テストビルド待ち）|
| 最新テスト件数 | 1261 件 全 PASS(dirty-switch-guard 時点、v254〜v259 で +81)|
...
- **現在ブランチ**: `feature/settings-scope-clarity`（main `8eecebb` から分岐、実装 commit STEP1〜4 `29ad4a3`/`d82e982`/`fe3d464`/`64caa8f` / バグ①修正 `d9bd78b` / 切替ガード `504e35c`、未 push）
- **直前 commit**: `504e35c fix(blinds): 未保存編集中のトーナメント切替に破棄/やめる確認を追加（嘘ラベル+黙って再ポイント根治）`
...
- **配信状況**: **v2.5.0 配信済み（最新・公開中）**。GitHub Release 公開済、自動更新有効。v2.5.1（settings-scope-clarity STEP1〜4）はテストビルド済・前原実機 6-B 待ち → GO で配信
- **次のアクション(想定)**:
