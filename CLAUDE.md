...
> - **★実験(2026-07-07〜・このプロジェクトのみ)**: 完了レビュー実行時、まず `{ scriptPath: "C:\\Users\\user\\.claude\\workflows\\cc-review2.js", args: {同上} }` で共通版を実行する(ユーザーレベル1箇所管理の実験。※ name 指定 `cc-review2` は解決されないと確認済=必ず scriptPath で。懐疑役の観点は当プロジェクトの `.claude/cc-review-verify.md` を実行時に読む)。失敗したら従来の `cc-review` にフォールバックし、**どちらで動いたかを完了報告に1行明記**する(結果次第で全プロジェクト移行判断)。
