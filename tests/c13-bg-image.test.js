...
test('T10: IPC display:selectBackgroundImage が main + preload に橋渡しされている', () => {
  ...
  // preload bridge
  // v2.1.18-meas1: preload.js は perf:ipc:roundtrip 計測のため `_measuredInvoke` ラッパ経由に変更
  //   （内部で ipcRenderer.invoke(channel, ...args) を呼ぶ形は維持、チャンネル名・引数は無変更）。
  //   どちらの形式でも bridge が存在することを許容。
  assert.match(PRELOAD, /selectBackgroundImage\s*:\s*\(\)\s*=>\s*(?:ipcRenderer\.invoke|_measuredInvoke)\(\s*['"]display:selectBackgroundImage['"]/,
...
test('T15: form-dialog--tabs に display: flex / overflow: hidden が再発していない（rollback 維持）', () => {
  ...
  // settings-scope-clarity STEP3→blinds-table-fix: 既定高 700px→920px→1100px（大画面でレベル表 0 段潰れ根治）。
  //   「初期 height（min(...px) 形式の固定値）が消失していない＝flex に戻していない」回帰防止の趣旨は維持。
  assert.match(tabsBlock[0], /height:\s*min\(1100px/,
    '.form-dialog--tabs の初期 height が消失（blinds-table-fix で min(1100px, 92vh) 固定、ハンドル可視確保）');
});
