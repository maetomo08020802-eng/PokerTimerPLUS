...
test('T15: form-dialog--tabs に display: flex / overflow: hidden が再発していない（rollback 維持）', () => {
  ...
  // settings-scope-clarity STEP3: 既定高を 700px → 920px に引き上げ（ブラインドのレベル表 10〜12 段表示）。
  //   「初期 height（min(...px) 形式の固定値）が消失していない＝flex に戻していない」回帰防止の趣旨は維持。
  assert.match(tabsBlock[0], /height:\s*min\(920px/,
    '.form-dialog--tabs の初期 height が消失（STEP3 で min(920px, 92vh) 固定、ハンドル可視確保）');
});
