...
test('T9 (Fix 5): applyHallPreStartState で isPaused 受信 + hallPreStartState.isPaused 更新', () => {
  ...
  assert.match(body, /const\s+isPaused\s*=\s*!!payload\.isPaused/,
    'applyHallPreStartState 内で `const isPaused = !!payload.isPaused` 抽出経路がない（Fix 5 未実装）');
...
// ============================================================
test('T11 (Fix 5): style.css に「一時停止中」ラベル CSS セレクタ + content が存在', () => {
  // セレクタ
  assert.match(STYLE_CSS, /\[data-role="hall"\]\s*\.clock\[data-prestart-paused="true"\]::after/,
    'style.css に [data-role="hall"] .clock[data-prestart-paused="true"]::after セレクタがない（Fix 5 未実装）');
  // content: '一時停止中'
  // セレクタ直後のブロック内に content: '一時停止中'
  ...
  assert.match(blockMatch[1], /content:\s*['"]一時停止中['"]/,
    'CSS の content プロパティに「一時停止中」が含まれていない');
});
...
// ============================================================
// T14 (Fix 7): package.json version が 2.1.15（rc1 サフィックスなし）
// ============================================================
test('T14 (Fix 7): package.json version が 2.1.15', () => {
  assert.equal(PKG.version, '2.1.15', `package.json version が 2.1.15 ではない（実際: ${PKG.version}）`);
});
