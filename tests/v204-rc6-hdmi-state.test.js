...
// ============================================================
test('Fix 2-A: switchOperatorToSolo は mainWindow.minimize を呼び mainWindow.close を呼ばない', () => {
  const body = extractFunctionBody(MAIN, /async function switchOperatorToSolo\s*\(\s*\)\s*\{/);
  assert.ok(body, 'switchOperatorToSolo が見つからない');
  assert.match(body, /mainWindow\.minimize\s*\(\s*\)/,
    'switchOperatorToSolo が mainWindow.minimize を呼んでいない');
  assert.doesNotMatch(body, /mainWindow\.close\s*\(\s*\)/,
    'switchOperatorToSolo が mainWindow.close を呼んでいる（rc6 で禁止）');
  // hall 側は close する
  assert.match(body, /hallWindow\.close/,
    'switchOperatorToSolo で hallWindow.close を呼んでいない（hall 側は閉じる必要あり）');
});

test('Fix 2-A: switchOperatorToSolo が _showRestoreNoticeOnce フラグを立てる', () => {
  const body = extractFunctionBody(MAIN, /async function switchOperatorToSolo\s*\(\s*\)\s*\{/);
  assert.ok(body, 'switchOperatorToSolo が見つからない');
  assert.match(body, /_showRestoreNoticeOnce\s*=\s*true/,
    'switchOperatorToSolo で _showRestoreNoticeOnce フラグ立てなし');
});

test('Fix 2-B: createOperatorWindow に restore イベント + ポップアップ案内', () => {
  const body = extractFunctionBody(MAIN, /function createOperatorWindow\s*\([^)]*\)\s*\{/);
  assert.ok(body, 'createOperatorWindow が見つからない');
  // win.on('restore', ...) ハンドラがある
  assert.match(body, /win\.on\(\s*['"]restore['"]/,
    'createOperatorWindow に win.on(\'restore\') なし');
  // dialog.showMessageBox で案内
  assert.match(body, /restore[\s\S]{0,500}showMessageBox/,
    'restore ハンドラで dialog.showMessageBox による案内なし');
  // ポップアップ文言（前原さん要望）
  assert.match(body, /AC\s*ウィンドウについて/,
    'ポップアップタイトル「AC ウィンドウについて」なし');
  assert.match(body, /閉じるとアプリも閉じます/,
    'ポップアップ本文「閉じるとアプリも閉じます」なし（前原さん要望文言）');
});
