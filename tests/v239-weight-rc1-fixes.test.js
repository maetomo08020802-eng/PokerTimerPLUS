...
test('T1: package.json.version === 2.1.20-rc1', () => {
  assert.equal(PKG.version, '2.10.0', `期待 2.1.20-rc1, 実際 ${PKG.version}`);
});
...
// ============================================================
test('T10: v2.2.1 — 計測機構（バッジ + meas2 6 カテゴリ + 症状確証 4 ラベル）完全撤去', () => {
  // v2.2.1: 試験ビルド時のみ skip（meas / rc サフィックス）
  if (/-(meas|rc)\d+(\.\d+)?$/.test(PKG.version || '')) return;
  // バッジ撤去
  assert.ok(!INDEX_HTML.includes('meas-build-badge'),
    'index.html に meas-build-badge が残存（v2.2.1 撤去違反）');
