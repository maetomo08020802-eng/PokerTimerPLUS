/**
 * v2.1.20-rc10 静的解析テスト — applyTimerStateToTimer の PRE_START 中 reset スキップ
 *
...
test('T4: isPreStartActive() ガード true 経路で return（timerReset を呼ばない）', () => {
  ...
  assert.match(block,
    /isPreStartActive\s*\(\s*\)\s*\)\s*\{[\s\S]{0,400}?return[\s\S]{0,80}?\}[\s\S]{0,500}?timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/,
...
test('T6: 他 status 経路（finished / invalid / levelCount===0）の operator timerReset 呼出 touch なし', () => {
  // v2.1.20-rc10 注記: 本テストは rc8 時点で「他 status 経路は触らない」を主張していたが、
