/**
 * v2.2.1 静的解析テスト — applyTimerStateToTimer の PRE_START 中 reset スキップ
 *
...
test('T1: package.json.version === 2.2.1', () => {
  assert.equal(PKG.version, '2.10.0', `期待 2.2.1, 実際 ${PKG.version}`);
});
...
test('T4: isPreStartActive() ガード true 経路で return（timerReset を呼ばない）', () => {
  const block = extractIdleNonHallBlock();
  assert.ok(block, '!isHallApply ブロックが抽出できない');
  // ガード ブロック内に return 存在 + その後に timerReset()（rc10: timerReset({ force: false }) も許容）
  // v2.2.1: ガード内に race window 計測 (_raceEntryMs/_raceExitMs) + race-window-entry ラベル
  //   発火経路追加で約 600 char に拡大、limit を 400 → 1200 に拡張
  assert.match(block,
...
test('T6: 他 status 経路（finished / invalid / levelCount===0）の operator timerReset 呼出 touch なし', () => {
  // v2.2.1 注記: 本テストは rc8 時点で「他 status 経路は触らない」を主張していたが、
  //   rc9 で意図的にスコープを拡張し、4 経路すべてに isPreStartActive() ガード + trigger フィールドを追加。
  //   よって rc9 以降は「finished 経路にも timerReset() 呼出が存在する（ただし PRE_START 中はスキップ）」を検証する。
  //   詳細な 4 経路ガード検証は v247（v247-skip-reset-all-routes.test.js）に集約済。
  const finishedStart = RENDERER.indexOf("if (ts.status === 'finished')");
  assert.ok(finishedStart >= 0, "finished 経路が見つからない");
  const finishedSlice = RENDERER.slice(finishedStart, finishedStart + 1500);
  // 'finished' 経路に timerReset(...) 呼出は引き続き存在する（ガード外、通常 IDLE 経路の挙動）
  //   rc10: 引数なし `timerReset()` または `timerReset({ force: false })` のいずれかを許容
  assert.match(finishedSlice, /timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/,
    "'finished' 経路に timerReset(...) 呼出が消失（通常 IDLE 経路の挙動を維持すべき）");
});
...
// ============================================================
test('T8: v2.2.1 — rc1〜rc6-meas3 機構保持確認（priority buffer 保持、rc6-meas3 計測機構撤去）', () => {
  // priority buffer（rc6-meas3 で追加、保持）
  assert.match(MAIN_JS, /const\s+PRIORITY_LOG_LABELS\s*=\s*new\s+Set\s*\(/, 'priority buffer Set 消失');
  // 撤去確認: rc6-meas3 計測 wrapper
...
// T10: 計測機構 + 新規 rc8 ラベル
// ============================================================
test('T10: v2.2.1 — meas/symptom/meas3 撤去 + rc2/rc4/rc5/rc7/rc8 edge ラベル保持', () => {
