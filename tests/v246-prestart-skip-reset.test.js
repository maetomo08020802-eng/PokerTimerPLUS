/**
 * v2.1.20-rc9 静的解析テスト — applyTimerStateToTimer の PRE_START 中 reset スキップ
 *
...
test('T6: 他 status 経路（finished / invalid / levelCount===0）の operator timerReset 呼出 touch なし', () => {
  // v2.1.20-rc9 注記: 本テストは rc8 時点で「他 status 経路は触らない」を主張していたが、
  ...
  assert.match(finishedSlice, /timerReset\s*\(\s*\)/,
