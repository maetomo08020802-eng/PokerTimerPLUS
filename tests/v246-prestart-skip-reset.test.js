/**
 * v2.1.20-rc10.1 静的解析テスト — applyTimerStateToTimer の PRE_START 中 reset スキップ
 *
...
test('T4: isPreStartActive() ガード true 経路で return（timerReset を呼ばない）', () => {
  ...
  // v2.1.20-rc10.1: ガード内に race window 計測 (_raceEntryMs/_raceExitMs) + race-window-entry ラベル
...
test('T6: 他 status 経路（finished / invalid / levelCount===0）の operator timerReset 呼出 touch なし', () => {
  // v2.1.20-rc10.1 注記: 本テストは rc8 時点で「他 status 経路は触らない」を主張していたが、
