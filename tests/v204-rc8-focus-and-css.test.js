...
test('Fix 4: onRoleChanged ハンドラ内で updateOperatorPane を即時呼出', () => {
  ...
  assert.match(m[0], /updateOperatorPane\s*\(\s*_lastTimerStateForRoleSwitch\s*\)/,
...
// version 同期確認（rc8）
// ============================================================
test('version: package.json は 2.0.4-rc18（rc18 第 1 弾 hall structure 同期 + operator-pane + ring buffer + 4 ラベル追従）', () => {
