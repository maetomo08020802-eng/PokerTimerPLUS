...
test('Fix 4: subscribe 内で _lastTimerStateForRoleSwitch に直近 state を保存', () => {
  ...
  // subscribe コールバック内で代入。v2.1.18-rc2: 冒頭の Fix 2-A hall:subscribe:fire 計測ログ追加で
  //   ウィンドウが大きくなったため 200 → 1500 に拡大（subscribe 冒頭の保険ログ + 既存代入を許容）。
  // v2.1.20-meas1: main subscribe が subscribeNamed('subscribe:main-renderer', ...) に変更されたため
  //   両形式（subscribe / subscribeNamed）を許容。
  assert.match(RENDERER,
    /subscribe(?:Named)?\(\s*(?:['"][^'"]*['"]\s*,\s*)?\(\s*state[^)]*\)\s*=>\s*\{[\s\S]{0,1500}?_lastTimerStateForRoleSwitch\s*=\s*state/,
