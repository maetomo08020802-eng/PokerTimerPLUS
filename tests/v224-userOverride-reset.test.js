...
test('T1 (Fix 1): subscribe 内で status 変化時に slideshowState.userOverride = "auto" リセット', () => {
  ...
  const subscribeIdx = RENDERER.indexOf('subscribe((state, prev) =>');
  assert.ok(subscribeIdx >= 0, 'subscribe コールバック開始位置が見つからない');
...
test('T6: package.json version は 2.1.12 + scripts.test に v224 登録', () => {
  assert.equal(PKG.version, '2.1.17-rc2',
    `package.json version が ${PKG.version}（期待 2.1.17-rc2）`);
