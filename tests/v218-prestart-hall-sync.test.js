...
test('T2 (Fix 1): main.js に ipcMain.on("dual:publish-pre-start-state") 登録', () => {
  ...
  // v2.1.20-rc10: cache merge ロジック追加で handler が長くなったため slice を 4000 に拡張
...
test('T6 (Fix 3): timer.js PRE_START 経路で新 handler 発火（5 箇所）', () => {
  ...
  // v2.1.20-rc10: reset シグネチャが `reset()` → `reset(opts = {})` に拡張されたため正規表現で吸収
...
test('T13: package.json version 2.1.12 + scripts.test に v218 登録', () => {
  assert.equal(PKG.version, '2.1.20-rc10', `version が ${PKG.version}（期待 2.1.18）`);
