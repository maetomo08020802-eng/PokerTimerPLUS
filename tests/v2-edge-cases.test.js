...
// ============================================================
// T3: display-removed で operator 側の display が抜けたケース — hallWindow 不在/非対象なら何もしない
// ============================================================
test('T3: display-removed で hallWindow 不在 or 非対象 display は何もしない（operator 側除外）', () => {
  ...
  // hallWindow 不在 / destroyed の早期 return
  assert.match(removed, /!hallWindow\s*\|\|\s*hallWindow\.isDestroyed\(\)[\s\S]*?return/,
    'hallWindow 不在 / destroyed の early return なし');
  // isWindowOnDisplay で対象 display 上の hallWindow のみ処理（false なら何もしない）
  assert.match(removed, /isWindowOnDisplay\s*\(/, 'isWindowOnDisplay 判定なし（operator 側除外できない）');
});
...
// ============================================================
// T6: _dualStateCache のキーが 9 種類で固定されている（仕様の明示）
// ============================================================
test('T6: _dualStateCache のキーが期待の 9 種類のみ', () => {
