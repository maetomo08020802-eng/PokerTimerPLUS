...
test('B-1 cross-check: switchOperatorToSolo は close せず minimize（rc6 改修、_suppressCloseConfirm 不要）', () => {
  // v2.0.4-rc6 Fix 2: switchOperatorToSolo は operator を close → minimize に変更したため、
  //   _suppressCloseConfirm bypass は不要になった（hall 側のみ close、operator は close しない）。
  const body = extractFunctionBody(MAIN, /async function switchOperatorToSolo\s*\(\s*\)\s*\{/);
  assert.ok(body, 'switchOperatorToSolo が見つからない');
  // minimize していることを確認
  assert.match(body, /mainWindow\.minimize\s*\(\s*\)/,
    'switchOperatorToSolo が mainWindow.minimize を呼ばない（rc6 で minimize 化必須）');
  // mainWindow.close は呼ばない（重要、AC 残存 race の根本解消）
  assert.doesNotMatch(body, /mainWindow\.close\s*\(\s*\)/,
    'switchOperatorToSolo が mainWindow.close を呼んでいる（rc6 で minimize 化したため close は禁止）');
});
...
test('B-2: FORWARD_KEYS_FROM_HALL に基本操作キーが含まれる（rc4 で input.code ベースに変更）', () => {
  ...
  const items = m[1];
  // input.code ベース（rc3 の input.key ベース ' '/'r' は廃止）
  assert.match(items, /['"]Space['"]/,                'FORWARD_KEYS に Space なし');
  assert.match(items, /['"]Enter['"]/,                'FORWARD_KEYS に Enter なし');
  assert.match(items, /['"]Escape['"]/,               'FORWARD_KEYS に Escape なし');
  assert.match(items, /['"]KeyR['"]/,                 "FORWARD_KEYS に 'KeyR' なし");
  assert.match(items, /['"]KeyA['"]/,                 "FORWARD_KEYS に 'KeyA' なし");
  assert.match(items, /['"]KeyE['"]/,                 "FORWARD_KEYS に 'KeyE' なし（rc3 で無反応だったキー）");
  assert.match(items, /['"]KeyS['"]/,                 "FORWARD_KEYS に 'KeyS' なし");
  assert.match(items, /['"]KeyM['"]/,                 "FORWARD_KEYS に 'KeyM' なし");
  assert.match(items, /['"]KeyT['"]/,                 "FORWARD_KEYS に 'KeyT' なし");
  // 矢印キー
