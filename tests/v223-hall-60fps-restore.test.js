...
test('T3 (Fix B): renderHallTickFrame 関数定義 + 自己再帰 requestAnimationFrame', () => {
  ...
  // 自己再帰 rAF
  // v2.1.20-meas1: requestAnimationFrame → _wrappedRAF(_RafLabel.HALL_TICK_FRAME, ...) に置換、両形式許容
  assert.match(body, /hallTickState\.rafId\s*=\s*(?:requestAnimationFrame\s*\(\s*renderHallTickFrame\s*\)|_wrappedRAF\s*\(\s*_RafLabel\.HALL_TICK_FRAME\s*,\s*renderHallTickFrame\s*\))/,
    'renderHallTickFrame の自己再帰 rAF（requestAnimationFrame / _wrappedRAF どちらも可）がない');
});
...
test('T5 (Fix D): renderHallPreStartTick の rAF 自己再帰復活', () => {
  ...
  assert.match(body, /hallPreStartState\.rafId\s*=\s*(?:requestAnimationFrame\s*\(\s*renderHallPreStartTick\s*\)|_wrappedRAF\s*\(\s*_RafLabel\.HALL_PRE_START_TICK\s*,\s*renderHallPreStartTick\s*\))/,
    'renderHallPreStartTick の自己再帰 rAF（requestAnimationFrame / _wrappedRAF どちらも可）が復活していない');
});
...
test('T8 (v2.1.10 Fix 1 保持): timer.js 関数呼出の !isHallApply ガード完全保持', () => {
  ...
  // v2.1.20-rc10.1: idle 経路は isPreStartActive ガード追加のためブロック化された。
  //   inline `!isHallApply) timerReset()` と ブロック化 `!isHallApply) { ... timerReset(...) ... }` の両方を許容、合計 4 件以上。
  //   さらに rc10 で `timerReset({ force: false })` 引数付き呼出も追加されたため、両形式を許容。
  const inlineGuarded = body.match(/!\s*isHallApply\s*\)\s*timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/g) || [];
  const blockGuardedCount = (() => {
    let count = 0;
    const re = /!\s*isHallApply\s*\)\s*\{/g;
    let m;
    while ((m = re.exec(body)) !== null) {
      ...
      const blk = body.slice(braceOpen + 1, end - 1);
      if (/timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/.test(blk)) count++;
    }
    return count;
  })();
  const totalGuarded = inlineGuarded.length + blockGuardedCount;
  assert.ok(totalGuarded >= 4,
    `timerReset の !isHallApply ガード付き（inline ${inlineGuarded.length} + block ${blockGuardedCount}）が 4 件以上必要、現在合計 ${totalGuarded}件`);
