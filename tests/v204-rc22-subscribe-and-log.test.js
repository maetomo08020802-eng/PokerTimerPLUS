...
// v2.1.18-meas1: Ctrl+Shift+L ハンドラに meas:capture スナップショット保存ロジックが追加され
//   nested try/catch が増えたため、非貪欲 `[\s\S]*?\}\s*\)\s*;` regex は handler 冒頭の
//   `rollingLog('ui:keypress', {...});` で早期マッチしてしまう。balanced-brace 抽出で修正。
function _extractCtrlShiftLBlock(body) {
  const startRe = /globalShortcut\.register\s*\(\s*['"]CommandOrControl\+Shift\+L['"]\s*,\s*async\s*\(\s*\)\s*=>\s*\{/;
  const m = body.match(startRe);
  if (!m) return null;
  const startIdx = m.index;
  // m[0] は `... async () => {` まで。最後の `{` 位置を探して balanced 検索開始
  const openBraceIdx = startIdx + m[0].length - 1;   // 最後の `{` の index
  let depth = 1;
  ...
  // i は閉じ `}` の次。さらに `)` `;` を含めてスライス
  const endIdx = body.indexOf(';', i);
  return body.slice(startIdx, endIdx + 1);
}

test('T5: Ctrl+Shift+L ハンドラ内に await _flushRollingLog() 呼出が存在（rc18 I/O 順序保証）', () => {
  const body = extractFunctionBody(MAIN, /function\s+registerShortcuts\s*\(\s*\)\s*\{/);
  assert.ok(body, 'registerShortcuts 関数本体が見つからない');
  // v2.1.18-meas1: balanced-brace 抽出（nested try/catch 増加に対応）
  const block = _extractCtrlShiftLBlock(body);
  assert.ok(block, 'Ctrl+Shift+L 登録ブロック全体が抽出できない');
...
test('T6: Ctrl+Shift+L ハンドラ内に shell.openPath 呼出が存在', () => {
  ...
  assert.match(block, /_resolveLogsDir\s*\(\s*\)/,
...
test('rc18: ROLLING_LOG_BUFFER_MAX 5000 / _rollingLogBuffer 配列定義が維持されている', () => {
  // v2.1.20-rc6-meas3: ROLLING_LOG_BUFFER_MAX は計測ビルド時 50000 / 本番 5000 の三項演算に置換、リテラル位置を緩和
  assert.match(MAIN, /const\s+ROLLING_LOG_BUFFER_MAX\s*=[\s\S]{0,200}?5000\b/,
    'ROLLING_LOG_BUFFER_MAX = 5000（本番値）が見つからない');
...
test('version: package.json は 2.0.11', () => {
  ...
  assert.equal(pkg.version, '2.1.20-rc6-meas3',
