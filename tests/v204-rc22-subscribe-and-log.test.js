...
// ============================================================
// 計測ラベル 8 件維持確認（rc22 第 2 弾で削除しない、rc23 削除）
// ============================================================

test('計測ラベル: renderer:onRoleChanged: 系 6 件が renderer.js に存在', () => {
  const labels = [
  ...
  for (const lbl of labels) {
    assert.ok(RENDERER.includes(lbl),
      `計測ラベル '${lbl}' が renderer.js に見つからない（rc22 第 2 弾で削除しない、rc23 削除予定）`);
  }
});

test('計測ラベル: preload:onRoleChanged: 系 2 件が preload.js に存在', () => {
  const labels = [
  ...
  for (const lbl of labels) {
    assert.ok(PRELOAD.includes(lbl),
      `計測ラベル '${lbl}' が preload.js に見つからない（rc22 第 2 弾で削除しない、rc23 削除予定）`);
  }
});
