...
test('phase2c: ±30秒（adjustTimeBy）がエンジン・操作盤ボタン・キーボード・ヘルプに配線されている', () => {
  ...
  assert.ok(code['multi-control.js'].includes("'ArrowUp'") && code['multi-control.js'].includes("'ArrowDown'"),
    'キーボード ↑↓（30秒戻す/進める）の割当がない');
  assert.ok(/30秒戻す \/ 30秒進める/.test(code['multi-grid.js']), 'grid ヘルプに ↑↓ の説明がない');
  assert.ok(/↑↓キーは30秒単位/.test(sources['multi-control.html']), '操作盤案内に ↑↓=30秒の説明がない');
});
