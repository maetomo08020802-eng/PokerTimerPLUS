...
// ============================================================
// 13. Phase 3a: 堅牢化・エッジケース
// ============================================================
test('phase3a: 走行中差し替えの誤操作ガード（進行中のみ confirm・未割当ガード・キャンセルで巻き戻し）', () => {
  const c = code['multi-control.js'];
  assert.ok(/pane\.engine \? pane\.engine\.getRecord\(\) : null/.test(c), '未割当（engine null）ガードがない');
  assert.ok(c.includes('現在の進行状況（タイマー・人数など）は失われます'), '差し替え確認の文言がない');
  assert.ok(/pane\.els\.select\.value = pane\.tournamentId \|\| ''/.test(c), 'キャンセル時の select 巻き戻しがない');
...
test('phase3a: マルチ専用 HDMI 追従（独立リスナー・picker なし・hide 方式・既存 dual 経路無改変）', () => {
  const m = MAIN.match(/function setupMultiDisplayChangeListeners[\s\S]*?\r?\n\}\s*\r?\n\s*function toggleFullScreen/);
  assert.ok(m, 'マルチ専用 display リスナーがない');
  const b = m[0];
  // 遅延登録: モジュールロード時に screen へ触れない（electron stub 経由テストの偽 PASS 罠回避）+
  // enterMultiMode 初回のみ登録（idempotent フラグ）
  assert.ok(!/^setupMultiDisplayChangeListeners\(\);/m.test(MAIN), 'モジュールロード時に登録している（遅延登録の原則違反）');
  assert.ok(/_multiModeActive = true;\s*\n\s*setupMultiDisplayChangeListeners\(\);/.test(MAIN), 'enterMultiMode からの遅延登録がない');
  assert.ok(b.includes('_multiDisplayListenersInstalled'), '多重登録防止フラグがない');
  assert.ok((b.match(/if \(!_multiModeActive\) return;/g) || []).length >= 2, '両リスナーに _multiModeActive ガードがない');
  assert.ok(!b.includes('chooseHallDisplayInteractive'), 'マルチ追従が picker を出している（自動復帰の原則違反）');
  assert.ok(b.includes('multiGridWindow.hide()'), 'display-removed で grid を hide していない');
  assert.ok(!/multiGridWindow\.close/.test(b), 'grid を close している（closed→exitMultiMode 連鎖でモードごと落ちる）');
  assert.ok(/_multiDisplayRemovedAtMs/.test(b) && /_multiDisplayAddedAtMs/.test(b), '多重発火 debounce（専用時間窓）がない');
  assert.ok(!/_displayAddedPending|_displayRemovedPending/.test(b), '既存 dual 用 debounce フラグを共有している');
  // 既存 setupDisplayChangeListeners 側は無改変（multi 分岐が混入していない + 既存ガード維持）
  const dual = MAIN.match(/function setupDisplayChangeListeners[\s\S]*?function createMainWindow/)[0];
  assert.ok(!dual.includes('multiGridWindow'), '既存 dual リスナーに multi 分岐が混入している');
  assert.ok(/if \(!hallWindow \|\| hallWindow\.isDestroyed\(\)\) return;/.test(dual), '既存 display-removed の hallWindow ガードが消えた');
  assert.ok(/if \(_multiModeActive\) return;/.test(dual), '既存 display-added の _multiModeActive ガードが消えた');
});

test('phase3a: 復元キャンセルの専用文言（reason 発行 → operator 表示の両側配線）', () => {
  assert.ok(/reason: 'restore-cancelled'/.test(MAIN), 'main が restore-cancelled reason を発行していない');
...
test('phase2f追補2: モード開始時の確認モーダル（データ非保存の明示・復元検出より前・キャンセルで開始中止）', () => {
  ...
  assert.ok(b.includes('保存・上書きされません'), '開始確認モーダルにデータ非保存の明示文言がない');
  assert.ok(b.includes('マルチ表示モード（4分割）を開始しますか'), '開始確認モーダルの message がない');
  // 開始確認 → 復元検出（Phase 2e）の順（確認より先に復元ダイアログが出ない）
  const enterBody = b.match(/async function enterMultiMode[\s\S]*?\n\}/)[0];
  assert.ok(enterBody.indexOf('開始しますか') < enterBody.indexOf('_readMultiSession'),
    '開始確認モーダルが復元検出より後にある');
  // キャンセルは picker と同じ reason 'cancelled'（operator 側は無言で戻る既存経路 = renderer 無改変）
  assert.ok(/startChoice !== 0\) return \{ ok: false, reason: 'cancelled' \}/.test(enterBody),
    'キャンセルで開始中止（reason cancelled）になっていない');
});
