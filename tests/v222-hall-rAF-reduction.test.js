...
test('T2 (Fix 1): timerReset / timerAdvanceBy / timerPause すべてに hall ガード', () => {
  const body = extractFnBody(RENDERER, /function\s+applyTimerStateToTimer\s*\([^)]*\)\s*\{/);
  assert.ok(body, 'applyTimerStateToTimer 関数本体が抽出できない');
  // timerReset の全呼出箇所が hall ガード（直接 inline or ブロック内）に囲まれていること
  // v2.1.20-rc10.1: timerReset 呼出は引数なし `timerReset()` と `timerReset({ force: false })` の両方を許容
  //   （rc10 で applyTimerStateToTimer 4 経路は `{ force: false }` 経由に切替えられた多層防御版）
  const timerResetCalls = body.match(/timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/g) || [];
  ...
  //   ({ ... if (...) return; timerReset(); }）。inline `!isHallApply) timerReset()` パターンと
  //   ブロック化パターン `!isHallApply) { ... timerReset(...) ... }` の両方を許容、合計 4 件以上。
  const inlineGuarded = body.match(/!\s*isHallApply\s*\)\s*timerReset\s*\(\s*(?:\{\s*force\s*:\s*false\s*\}\s*)?\)/g) || [];
  const blockGuardedCount = (() => {
    // !isHallApply) { ... } ブロックを balanced brace で抽出し、ブロック内に timerReset(...) を含むものを数える
    let count = 0;
  ...
  assert.ok(totalGuarded >= 4,
    `timerReset 呼出で !isHallApply ガード付き（inline ${inlineGuarded.length} + block ${blockGuardedCount}）が 4 件以上必要（現在合計 ${totalGuarded}件）`);
