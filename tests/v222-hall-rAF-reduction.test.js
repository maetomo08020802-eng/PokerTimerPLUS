...
// ============================================================
test('T4 (Fix 2): renderHallPreStartTick の独立 rAF 駆動部分が削除', () => {
  // 旧名 renderHallPreStartTick が requestAnimationFrame(renderHallPreStartTick) 自己参照
  // を持たないこと（再帰 rAF 廃止）
  // 新名 renderHallPreStartFrame でも、関数内に再帰 requestAnimationFrame がないこと
  const oldFnBody = extractFnBody(RENDERER, /function\s+renderHallPreStartTick\s*\([^)]*\)\s*\{/);
  const newFnBody = extractFnBody(RENDERER, /function\s+renderHallPreStartFrame\s*\([^)]*\)\s*\{/);
  // 旧名 / 新名のいずれかが存在し、再帰 rAF を持たない
  assert.ok(oldFnBody || newFnBody,
    'renderHallPreStartTick または renderHallPreStartFrame 関数本体が抽出できない');
  const rawBody = newFnBody || oldFnBody;
  // コメント（//... と /* ... */）を剥がしてから検査（v2.1.10 廃止コメント内の旧コード文字列を誤検知しないため）
  const body = rawBody
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\/\/[^\n]*/g, '');
  // 関数自身を再帰呼出する requestAnimationFrame パターンが無いこと
  assert.doesNotMatch(body, /requestAnimationFrame\s*\(\s*renderHallPreStartTick\s*\)/,
    'renderHallPreStartTick 関数内で requestAnimationFrame(renderHallPreStartTick) の再帰 rAF が残っている（v2.1.10 で廃止済の regression）');
  assert.doesNotMatch(body, /requestAnimationFrame\s*\(\s*renderHallPreStartFrame\s*\)/,
    'renderHallPreStartFrame 関数内で requestAnimationFrame(renderHallPreStartFrame) の再帰 rAF が残っている（v2.1.10 で廃止済の regression）');
  // 関数内で hallPreStartState.rafId への代入もないこと（コメント剥がし後）
  assert.doesNotMatch(body, /hallPreStartState\.rafId\s*=\s*requestAnimationFrame/,
    'renderHallPreStartFrame/Tick 内で hallPreStartState.rafId に rAF 代入がある（v2.1.10 で廃止済の regression）');
});
...
// ============================================================
// T7: package.json version 2.1.10 + scripts.test に v222 登録
// ============================================================
test('T7: package.json version は 2.1.10 + scripts.test に v222 登録', () => {
