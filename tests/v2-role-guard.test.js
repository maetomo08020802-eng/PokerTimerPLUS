 ...
 *   - notifyOperatorAction の呼出が role === 'operator' 限定（条件分岐の存在）
...
// ============================================================
// T4: notifyOperatorAction の呼出が role === 'operator' 限定
// ============================================================
test('T4: notifyOperatorAction wrapper が role==="operator" のみで呼ばれる', () => {
  // ヘルパー関数の存在 + role ガード
  const helperRe = /function\s+notifyOperatorActionIfNeeded\s*\([^)]*\)\s*\{[\s\S]*?window\.appRole\s*!==\s*['"]operator['"]/;
  assert.match(RENDERER, helperRe,
    'notifyOperatorActionIfNeeded 内に window.appRole !== "operator" 早期 return ガードなし');
  // 主要操作で呼ばれている（btnStart / btnPause click handler）
  assert.match(RENDERER, /notifyOperatorActionIfNeeded\(\s*['"]timer:start['"]/, 'timer:start の通知呼出なし');
  assert.match(RENDERER, /notifyOperatorActionIfNeeded\(\s*['"]timer:pause['"]/, 'timer:pause の通知呼出なし');
});
