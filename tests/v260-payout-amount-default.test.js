...
test('S3 (renderer / v2.6.0): 配当入力は常に金額モード（% UI 撤去で payoutInputMode 常時 amount）', () => {
  // v2.6.0 STEP3: % 入力 toggle 撤去 → 編集開きは常に 'amount'
  assert.ok(/payoutInputMode\s*=\s*'amount'/.test(RENDERER),
    "payoutInputMode = 'amount'（金額固定）になっていない");
  assert.ok(!/getElementById\('js-tournament-payout-mode'\)/.test(RENDERER),
    '撤去したはずの payout-mode toggle 参照が残存');
});
...
test('S12: version 据え置き（実装中は bump しない）', () => {
  assert.equal(PKG.version, '2.10.0', `version が ${PKG.version}（実装中は 2.5.1 据え置き、配信時 2.5.2）`);
});
