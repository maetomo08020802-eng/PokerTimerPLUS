...
// ============================================================
test('T3: toggleFullScreen が getFocusedWindow ベースで両ウィンドウ対応', () => {
  const body = extractFunctionBody(MAIN, /function toggleFullScreen\s*\(\s*\)\s*\{/);
  assert.ok(body, 'toggleFullScreen が見つからない');
  // BrowserWindow.getFocusedWindow() を呼び出している
  assert.match(body, /BrowserWindow\.getFocusedWindow\s*\(\s*\)/,
    'toggleFullScreen が getFocusedWindow を使っていない（hall 対応なし）');
  // setFullScreen 呼出がある
  assert.match(body, /setFullScreen\s*\(/,
    'toggleFullScreen に setFullScreen 呼出なし');
  // mainWindow への fallback がある（既存挙動の保護）
  assert.match(body, /mainWindow/,
    'toggleFullScreen に mainWindow fallback なし（既存挙動の互換破壊リスク）');
});
