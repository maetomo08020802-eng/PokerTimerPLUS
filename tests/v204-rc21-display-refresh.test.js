...
test('T7: renderer.js に 6 件の renderer:onRoleChanged: 計測ラベル送信存在', () => {
  ...
  for (const label of expectedLabels) {
    assert.ok(handler.indexOf(label) >= 0,
      `onRoleChanged ハンドラに計測ラベル "${label}" が見つからない（タスク 2 不完全）`);
  }
});

test('T8: preload.js に preload:onRoleChanged:enter / :catch ラベル送信存在', () => {
  assert.ok(PRELOAD.indexOf('preload:onRoleChanged:enter') >= 0,
    'preload.js に preload:onRoleChanged:enter ラベル送信が見つからない');
  assert.ok(PRELOAD.indexOf('preload:onRoleChanged:catch') >= 0,
    'preload.js に preload:onRoleChanged:catch ラベル送信が見つからない');
  // rolling-log:write IPC 経由（既存の log.write 経路と同 IPC を使用）
  assert.match(PRELOAD, /ipcRenderer\.send\s*\(\s*['"]rolling-log:write['"]/,
    'preload.js に rolling-log:write IPC 経由のログ送信が見つからない');
});
