...
test('T7: ROLLING_LOG_BUFFER_MAX 定数が定義され値は 5000', () => {
  // v2.1.20-rc6-meas3: ROLLING_LOG_BUFFER_MAX は計測ビルド時 50000 / 本番 5000 の三項演算に置換。
  //   本番値 5000 は ternary の右辺として残存しているため、リテラル位置の制約は緩める。
  assert.match(MAIN, /const\s+ROLLING_LOG_BUFFER_MAX\s*=[\s\S]{0,200}?5000\b/,
    'ROLLING_LOG_BUFFER_MAX = 5000（本番値）の宣言が見つからない');
});
...
test('rc15 維持: 5 分保持 + 30 秒切捨間隔 定数が維持', () => {
  // v2.1.20-rc6-meas3: ROLLING_LOG_RETENTION_MS は計測ビルド時 30 分 / 本番 5 分の三項演算に置換、リテラル位置を緩和
  assert.match(MAIN, /ROLLING_LOG_RETENTION_MS\s*=[\s\S]{0,200}?5\s*\*\s*60\s*\*\s*1000/,
    '5 分保持期間定数（本番値）が消失');
...
test('rc18 削除確認: main.js 全体で fs.promises.appendFile が使われていない（ring buffer 化）', () => {
  // v2.1.20-rc6-meas3: priority buffer 用に _flushPriorityLog が fs.promises.appendFile を意図的に追加。
  //   rc18 のリングバッファ化は rolling-current.log（writeFile 経由）に限定された制約であり、
  //   別系統の priority-events.log は append が設計仕様（10000 行循環）。
  //   よって _flushPriorityLog 関数本体内 1 箇所のみ許容する。
  const appendOccurrences = (MAIN.match(/fs\.promises\.appendFile/g) || []).length;
  assert.ok(appendOccurrences <= 1,
    `main.js に fs.promises.appendFile が ${appendOccurrences} 件残存（priority buffer 用 1 件のみ許容、それ以外は rc18 違反）`);
  // 1 件残存時は _flushPriorityLog 内であること
  if (appendOccurrences === 1) {
    assert.match(MAIN, /async\s+function\s+_flushPriorityLog\s*\([\s\S]{0,500}?fs\.promises\.appendFile/,
      'fs.promises.appendFile が _flushPriorityLog 外で使用されている（rc18 違反）');
  }
});
