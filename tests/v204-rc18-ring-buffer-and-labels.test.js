...
test('T7: ROLLING_LOG_BUFFER_MAX 定数が定義され値は 5000', () => {
  // v2.1.20-rc8: ROLLING_LOG_BUFFER_MAX は計測ビルド時 50000 / 本番 5000 の三項演算に置換。
...
test('rc15 維持: 5 分保持 + 30 秒切捨間隔 定数が維持', () => {
  // v2.1.20-rc8: ROLLING_LOG_RETENTION_MS は計測ビルド時 30 分 / 本番 5 分の三項演算に置換、リテラル位置を緩和
...
test('rc18 削除確認: main.js 全体で fs.promises.appendFile が使われていない（ring buffer 化）', () => {
  // v2.1.20-rc8: priority buffer 用に _flushPriorityLog が fs.promises.appendFile を意図的に追加。
