...
test('T7 (Fix 5): package.json version が 2.1.16', () => {
  assert.equal(PKG.version, '2.1.16', `package.json version が 2.1.16 ではない（実際: ${PKG.version}）`);
});
...
// ============================================================
// T10: rc1 計測機構残骸（grep で 0 件、v2.1.15 撤去状態維持）
// ============================================================
test('T10: rc1 計測機構（meas:〜 6 ラベル + measurement-mode-badge）すべて 0 件維持', () => {
  const labels = [
  ...
  assert.ok(!INDEX_HTML.includes('measurement-mode-badge'),
    'index.html に measurement-mode-badge が残存');
  assert.ok(!STYLE_CSS.includes('measurement-mode-badge'),
    'style.css に measurement-mode-badge が残存');
  assert.ok(!RENDERER.includes('measurement-mode-badge'),
    'renderer.js に measurement-mode-badge が残存');
});
