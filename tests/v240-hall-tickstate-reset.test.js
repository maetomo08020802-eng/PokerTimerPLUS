...
test('T1: package.json.version === 2.1.20-rc3', () => {
  assert.equal(PKG.version, '2.10.0', `期待 2.1.20-rc3, 実際 ${PKG.version}`);
});
...
// ============================================================
test('T9: v2.2.1 — 計測機構（バッジ + meas2 + 症状確証 4）撤去 + rc2 hall:hallTickState:reset edge ラベルは保持', () => {
  // 試験ビルド（rc / meas）時は skip
  ...
  // 保持確認: rc2 hall:hallTickState:reset edge ラベル
  assert.ok(RENDERER.includes('hall:hallTickState:reset'),
    'rc2 edge ラベル hall:hallTickState:reset が消失（撤去禁止対象）');
  ...
  for (const tg of triggers) {
    assert.ok(RENDERER.includes(`'${tg}'`),
      `hall:hallTickState:reset trigger '${tg}' が消失`);
  }
});
